...
class ResultForm(tk.Toplevel):
    ...
    def _build_ui(self):
        ...
        ttk.Label(panel, text="收敛代", font=("", 10, "bold"), foreground="blue").grid(row=0, column=0, sticky="w")
        self.edit_gen.grid(row=0, column=1, sticky="w")
        ttk.Label(panel, text="最优值", font=("", 10, "bold"), foreground="blue").grid(row=0, column=2, sticky="w", padx=(12, 0))
        self.edit_maxvalue = ttk.Entry(panel, width=12, state="readonly")
        self.edit_maxvalue.grid(row=0, column=3, sticky="w")
        ttk.Label(panel, text="收敛时间(秒)", font=("", 10, "bold"), foreground="blue").grid(row=0, column=4, sticky="w", padx=(12, 0))
        self.edit_time = ttk.Entry(panel, width=10, state="readonly")
        self.edit_time.grid(row=0, column=5, sticky="w")
        ttk.Label(panel, text="程序运行时间(秒)", font=("", 10, "bold"), foreground="blue").grid(row=0, column=6, sticky="w", padx=(12, 0))
        self.edit_runtime = ttk.Entry(panel, width=10, state="readonly")
        self.edit_runtime.grid(row=0, column=7, sticky="w")
        ttk.Label(panel, text="初始覆盖率(%)", font=("", 10, "bold"), foreground="blue").grid(row=1, column=0, sticky="w", pady=(8, 0))
        ...
        ttk.Label(panel, text="优化后覆盖率(%)", font=("", 10, "bold"), foreground="blue").grid(row=1, column=2, sticky="w", padx=(12, 0), pady=(8, 0))
        ...
        ttk.Label(panel, text="中间平均移动距离", font=("", 10, "bold"), foreground="blue").grid(row=1, column=4, sticky="w", padx=(12, 0), pady=(8, 0))
        self.edit_movedist = ttk.Entry(panel, width=10, state="readonly")
        self.edit_movedist.grid(row=1, column=5, sticky="w", pady=(8, 0))
        ttk.Label(panel, text="DE平均移动距离", font=("", 10, "bold"), foreground="blue").grid(row=1, column=6, sticky="w", padx=(12, 0), pady=(8, 0))
        self.edit_de_dist = ttk.Entry(panel, width=10, state="readonly")
        self.edit_de_dist.grid(row=1, column=7, sticky="w", pady=(8, 0))
        ttk.Label(panel, text="优化平均移动距离", font=("", 10, "bold"), foreground="blue").grid(row=2, column=0, sticky="w", pady=(8, 0))
        self.edit_optimaldist = ttk.Entry(panel, width=10, state="readonly")
        self.edit_optimaldist.grid(row=2, column=1, sticky="w", pady=(8, 0))
        ttk.Label(panel, text="需移动节点数", font=("", 10, "bold"), foreground="blue").grid(row=2, column=2, sticky="w", padx=(12, 0), pady=(8, 0))
        self.edit_movednum = ttk.Entry(panel, width=10, state="readonly")
        self.edit_movednum.grid(row=2, column=3, sticky="w", pady=(8, 0))
        ttk.Label(panel, text="C/d", font=("", 10, "bold"), foreground="blue").grid(row=2, column=4, sticky="w", padx=(12, 0), pady=(8, 0))
        ...
        ttk.Button(btn_frame, text="确定", command=self.destroy).pack()
        self._setup_setters()
    ...
    def _cut_edge(self, offset: int):
        from core.params import FIELD_XA, FIELD_YA, FIELD_XB, FIELD_YB
        self.canvas.create_rectangle(
            FIELD_XA + offset - 40, FIELD_YA - 30,
            FIELD_XB + offset + 30, FIELD_YA,
            fill="white", outline="white",
        )
        self.canvas.create_rectangle(
            FIELD_XA + offset, FIELD_YB,
            FIELD_XB + offset, FIELD_YB + 30,
            fill="white", outline="white",
        )
        self.canvas.create_rectangle(
            FIELD_XA + offset - 30, FIELD_YA,
            FIELD_XA + offset, FIELD_YB,
            fill="white", outline="white",
        )
        self.canvas.create_rectangle(
            FIELD_XB + offset, FIELD_YA,
            FIELD_XB + offset + 20, FIELD_YB,
            fill="white", outline="white",
        )
    ...
    def _draw_bounds(self, offset: int):
        from core.params import FIELD_XA, FIELD_YA, FIELD_XB, FIELD_YB
        self.canvas.create_rectangle(
            FIELD_XA + offset, FIELD_YA,
            FIELD_XB + offset, FIELD_YB,
    ...
    def _draw_mobile_layer(self, mob: list, offset: int, lines_from_initial=True):
        radius = int(round(self.state.sense_range))
        for i in range(1, self.state.mobile_num + 1):
            m = mob[i]
            self.canvas.create_oval(
                m.x - radius + offset, m.y - radius,
                m.x + radius + offset, m.y + radius,
                fill="gray", outline="green",
            )
        for i in range(1, self.state.mobile_num + 1):
            m = mob[i]
            r = 3
        ...
        if lines_from_initial:
            for i in range(1, self.state.mobile_num + 1):
                ...
                self.canvas.create_line(
                    s.x + offset, s.y, m.x + offset, m.y, fill="black",
                )
    ...
    def _draw_column_initial(self, offset: int):
        ...
        self._draw_mobile_layer(init_mob, offset, lines_from_initial=False)

    def _draw_column_positions(self, mob: list, offset: int):
        self._draw_static_layer(offset)
        self._draw_mobile_layer(mob, offset, lines_from_initial=True)
